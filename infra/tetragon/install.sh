...
echo "========================================================"
echo "🛡️  DÉPLOIEMENT INFRASTRUCTURE TETRAGON & EBPF"
echo "========================================================"

echo "🚀 1. CLUSTER KIND"
if kind get clusters | grep -q "^ebpf-lab$"; then
    echo "⚠️  Cluster déjà présent."
else
...
echo "🛡️  4. ENREGISTREMENT CRD & POLITIQUES"
...
until kubectl get crd tracingpolicies.cilium.io > /dev/null 2>&1; do
    ...
    if [ $COUNT -ge $MAX_RETRIES ]; then
        echo "❌ Erreur : Le CRD n'a pas pu être installé à temps."
        exit 1
    fi
...
if [ -d "$PROJECT_INFRA/policies/" ]; then
    ...
    echo "✅ Politiques appliquées."
else
    echo "❌ Erreur : Dossier policies introuvable."
    exit 1
fi
